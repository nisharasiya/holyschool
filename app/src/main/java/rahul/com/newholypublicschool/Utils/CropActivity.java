...
public class CropActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		imageToCrop = (ImageView) findViewById(R.id.cropImageView);
	...
	private void onSavedClick() {
		try {
			...
			//currentImage = imageToCrop.getCroppedBitmap();
					SingleInstance.getInstance().recentBitmap = currentImage;
				setResult(RESULT_OK);
				finish();
		} catch (Throwable e) {
	...
	/*class DoBackgroungJob extends AsyncTask<Void, Bitmap,String>{
		...
		protected void onPostExecute(String result) {
			...
			if (result != null) {
				...
				ToastUtil.toastShort(CropActivity.this, "Insufficient storage on device");
				setResult(RESULT_CANCELED);
			}
			finish();
		}*/
